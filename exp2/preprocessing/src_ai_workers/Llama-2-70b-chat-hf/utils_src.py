# original code from Alizadeh et al. (2025)
...
default_metrics = {
    ...
    'recall': lambda y_t, y_p: recall_score(y_t, y_p, zero_division=np.nan, average='macro'),
    'precision': lambda y_t, y_p: precision_score(y_t, y_p, zero_division=np.nan, average='macro'),
    'f1': lambda y_t, y_p: f1_score(y_t, y_p, zero_division=np.nan, average='macro')
}
...
def map_label_to_completion(label: str, task_num: int, full_label: bool = True) -> str:
    ...
    if task_num == 1:
        ...
        if full_label:
            new_label = label.upper() if label.lower() != 'neither' else 'NEUTRAL'
        ...
        if full_label:
            new_label_mapping = {
                'economic': 'ECONOMY', 'morality': 'MORALITY', 'fairness and equality': 'FAIRNESS AND EQUALITY',
            ...
            new_label_mapping = {
                'economic': 'A', 'morality': 'B', 'fairness and equality': 'C',
    ...
    assert new_label != '', f"Label {label} could not be mapped to a completion"
