...
from agg_methods_crowd_kit_stan import get_aggregation_methods, get_human_only_aggregation_methods
from io_utils import get_accuracy, get_recall

## Create result file
RESULT_CSV_PATH = f"{TARGET}_aggregation_results_crowd_kit_stan.csv"
if not WRITE_APPEND:
    ...
    for ai_name, num_ai in inst.iter_rows():
        ...
        if num_ai != 0:
            for iter in range(MAX_ITER):
                ...
                agg_methods = get_aggregation_methods(list(LABELS.keys()), random_state=iter)
                for method in agg_methods:
                    print(f"Method: {method.name}, Iteration: {iter}")
                    ret = method.fit_predict(human, ai, human_only=False)
            ...
            for iter in range(MAX_ITER):
                ...
                # get aggregation methods
                agg_methods = get_human_only_aggregation_methods(list(LABELS.keys()), random_state=iter)
                for method in agg_methods:
                    print(f"Method: {method.name}, Iteration: {iter}")
                    ret = method.fit_predict(df, ai=None, human_only=True)
                    acc = get_accuracy(ret, gt.set_index("task"))
                    recalls = []
                    for key in LABELS.keys():
                        if key in EXCEPT_LABELS:
