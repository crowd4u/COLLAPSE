...
WRITE_APPEND = False
WORKDIR = r"/app/exp2/main_scripts/ds1task5/workdir"
...
from agg_methods_truth_infer import get_aggregation_methods
from io_utils import get_accuracy, get_recall

## Create result file
RESULT_CSV_PATH = f"{TARGET}_aggregation_results_truth_infer.csv"
if not WRITE_APPEND:
    ...
    for ai_name, num_ai in inst.iter_rows():
        ...
        if num_ai != 0:
            for iter in range(MAX_ITER):
                ...
                df = pd.concat([human, ai], axis=0).drop(columns=["gt"])
                # get aggregation methods
                agg_methods = get_aggregation_methods(WORKDIR)
                for method in agg_methods:
                    print(f"Method: {method.name}, Iteration: {iter}")
                    ret = method.fit_predict(df, iter=iter, ds_name=f"__ds_{ai_name}_{num_ai}__")
            ...
            for iter in range(MAX_ITER):
                ...
                # get aggregation methods
                agg_methods = get_aggregation_methods(workdir=WORKDIR)
                for method in agg_methods:
                    print(f"Method: {method.name}, Iteration: {iter}")
                    ret = method.fit_predict(df, iter=iter, ds_name=f"__ds_{ai_name}_{num_ai}__")
                    acc = get_accuracy(ret, gt.set_index("task"))
